...
module.exports = class Bundle {
    ...
    addHelper(specifier, location) {
        const helperLocation = join(this.options.dist, location);
        const nameArray = helperLocation.split(sep);
        const name = nameArray[nameArray.length - 2];
        console.log(`Adding helper "${name}"`);
        this.helpers.push({ name, location });
    }
    //
    ...
    async discoverTemplates() {
        ...
        await Promise.all(
            templates.map(([specifier, location, path, source]) => {
